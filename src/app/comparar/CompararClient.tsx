...
export default function CompararClient() {
    ...
    if (compareList.length === 0) {
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center p-8 text-center space-y-6 transition-colors duration-300">
    ...
    return (
        <div className="container mx-auto max-w-7xl pt-10 pb-20 px-4 md:px-0 space-y-12 transition-colors duration-300">
            <header className="flex flex-col md:flex-row justify-between items-center gap-8 border-b border-black/5 pb-12">
                <div className="space-y-4 text-center md:text-left">
                    <h1 className="text-4xl sm:text-6xl font-black tracking-tighter text-zinc-900 font-heading">
            ...
            <div className="flex flex-row overflow-x-auto snap-x snap-mandatory pb-8 -mx-4 px-4 gap-6 md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-8 md:overflow-visible md:pb-0 md:mx-0 md:px-0 scrollbar-hide">
                {compareList.map((property) => {
                    const features = property.features || {}
                    return (
                        <div key={property.id} className="snap-center shrink-0 w-[85vw] md:w-auto bg-white border border-black/5 rounded-[3rem] overflow-hidden flex flex-col group hover:border-blue-500/30 transition-all duration-500 shadow-sm h-full">
                            <div className="h-64 relative overflow-hidden">
                                ...
                                <button
                                    ...
                                    className="absolute top-6 right-6 h-10 w-10 bg-black/50 backdrop-blur-md text-white rounded-full flex items-center justify-center hover:bg-red-500 transition-colors z-20"
                                >
                                    <Trash2 className="h-4 w-4" />
                                </button>
                            </div>
                            ...
                            <div className="p-10 space-y-10 flex-1 flex flex-col">
                                <div className="space-y-4">
                                    ...
                                    <h3 className="text-2xl font-black text-zinc-900 group-hover:text-blue-600 transition-all line-clamp-2">
                                        {property.title}
                                    </h3>
                                </div>
                                ...
                                <div className="pt-4 mt-auto">
                                    <Link href={`/propiedades/${property.id}`} className="block">
                                        <Button className="w-full h-14 rounded-2xl bg-zinc-900 text-white hover:bg-zinc-800 font-black uppercase tracking-widest text-[10px]">
                                            Ver Ficha Completa
                                        </Button>
                                    </Link>
                                </div>
                ...
                {compareList.length < 3 && Array.from({ length: 3 - compareList.length }).map((_, i) => (
                    <Link key={`empty-${i}`} href="/propiedades" className="snap-center shrink-0 w-[85vw] md:w-auto border-2 border-dashed border-black/5 rounded-[3rem] flex flex-col items-center justify-center p-12 text-center group hover:bg-black/[0.01] transition-all min-h-[500px]">
